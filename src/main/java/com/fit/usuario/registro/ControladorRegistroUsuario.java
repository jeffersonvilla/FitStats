...
public class ControladorRegistroUsuario {
	...
	public void registrarUsuario(String nombre, String email, char[] password) {
		if (validarDatosUsuario(nombre, email, password)) {
			if (dao.validarEmailDisponible(email)) {
				if (dao.crearUsuario(
						new Usuario(nombre, email, BCrypt.withDefaults().hashToString(BCrypt.MIN_COST, password)))) {
					...
					vista.problemasRegistrandoUsuario(Usuario.MENSAJE_PROBLEMA_REGISTRANDO_USUARIO);
			} else
				vista.validarEmail(Usuario.MENSAJE_EMAIL_EN_USO);
		}
	}
	...
	private boolean validarNombre(String nombre) {
		if (nombreTieneFormatoValido(nombre))
			return true;
	...
	private boolean validarPassword(char[] password) {
		if (passwordTieneFormatoValido(password))
			return true;
